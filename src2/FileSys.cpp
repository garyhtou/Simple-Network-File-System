...
{   
    ...
    // if not get error from error_array[0]

    //if file check that file exists
    //if not get error from error_array[3]

    //check that directory isempty
    //if not get error from error_array[7]
    //error_string = error_array[7];
}
